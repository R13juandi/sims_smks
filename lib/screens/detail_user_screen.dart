...
class _DetailUserScreenState extends State<DetailUserScreen> {
  ...
  @override
  void dispose() {
    _namaController.dispose(); _hpController.dispose(); _alamatController.dispose(); _nikController.dispose(); _agamaController.dispose(); _jkController.dispose(); _kelasController.dispose(); _nisnController.dispose(); _nipdController.dispose(); super.dispose();
  }
  ...
  Future<void> _updateDataUser() async {
    ...
    try {
      final Map<String, dynamic> updatePayload = {
        'full_name': _namaController.text.trim(), 'nomor_hp': _hpController.text.trim(), 'alamat': _alamatController.text.trim(), 'nik': _nikController.text.trim(), 'agama': _agamaController.text.trim(), 'jenis_kelamin': _jkController.text.trim(),
      };
      if (widget.userData['role'] == 'siswa') {
        updatePayload['kelas'] = _kelasController.text.trim(); updatePayload['nisn'] = _nisnController.text.trim(); updatePayload['nipd'] = _nipdController.text.trim();
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: Colors.white, elevation: 0, leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, color: Colors.black, size: 20), onPressed: () => Navigator.pop(context, false)),
        actions: [
          if (!_isEditing && _currentUserRole == 'tata_usaha') IconButton(icon: const Icon(Icons.edit_note_rounded, color: Color(0xFF1E40AF), size: 28), onPressed: () => setState(() => _isEditing = true)),
          if (_currentUserRole == 'tata_usaha') IconButton(icon: const Icon(Icons.delete_forever_rounded, color: Colors.redAccent), onPressed: _hapusUser),
        ],
      ),
      body: _isSaving
              ...
              child: Column(
                ...
                children: [
                  Center(
                    child: Column(
                      children: [
                        CircleAvatar(radius: 40, backgroundColor: role == 'siswa' ? const Color(0xFFDBEAFE) : const Color(0xFFDCEFDC), child: Icon(role == 'siswa' ? Icons.school : Icons.badge_rounded, size: 40, color: role == 'siswa' ? const Color(0xFF1E40AF) : Colors.green)),
