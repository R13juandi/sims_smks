...
class _ManajemenUserScreenState extends State<ManajemenUserScreen>
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(icon: Icon(Icons.school_rounded), text: 'Folder Siswa'),
            Tab(
        ...
        children: [
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Cari nama, kelas, atau NISN...',
          ...
          Expanded(
            child: _isLoading
                ...
                : TabBarView(
                    controller: _tabController,
                    children: [_buildTabSiswa(), _buildUserList('guru')],
                  ),
          ),
        ],
      ),
  ...
  // Menampilkan Folder Kelas untuk Siswa jika tidak ada pencarian
  Widget _buildTabSiswa() {
    if (_searchQuery.isNotEmpty) {
      return _buildUserList('siswa'); // Tampilkan hasil pencarian langsung
    }

    return ListView.builder(
      padding: const EdgeInsets.all(16),
      itemCount: _listKelasFolder.length,
          ...
          child: ListTile(
            ...
            onTap: () async {
              final result = await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => FolderSiswaDetailScreen(
                    kelas: kelas,
                    allUsers: _allUsers,
                  ),
                ),
  ...
  // Tampilan List Biasa (Untuk Guru atau Hasil Pencarian Siswa)
  Widget _buildUserList(String role) {
    ...
    if (filteredList.isEmpty) {
      return Center(
        child: Text(
          'Tidak ada data ${role == 'siswa' ? 'Siswa' : 'Guru'} ditemukan',
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(
              ...
              child: Icon(
                role == 'siswa' ? Icons.person : Icons.badge_rounded,
                color: role == 'siswa'
            ...
            onTap: () async {
              final result = await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => DetailUserScreen(userData: user),
                ),
              );
              if (result == true) _fetchUsers();
            },
...
// ------------------------------------------------------------------------
// SCREEN BARU: Untuk menampilkan siswa spesifik di dalam folder kelas
// ------------------------------------------------------------------------
class FolderSiswaDetailScreen extends StatelessWidget {
  final String kelas;
  final List<dynamic> allUsers;

  const FolderSiswaDetailScreen({
    super.key,
    required this.kelas,
  ...
  @override
  Widget build(BuildContext context) {
    // Filter siswa sesuai kelas yang diklik
    final siswaDiKelas = allUsers
        .where((u) => u['role'] == 'siswa' && u['kelas'] == kelas)
        .toList();
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      // Jika data dihapus/edit, kembalikan 'true' agar halaman utama refresh
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) =>
                              DetailUserScreen(userData: user),
                        ),
                      ).then((result) {
