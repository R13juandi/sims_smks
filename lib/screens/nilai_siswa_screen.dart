...
class _NilaiSiswaScreenState extends State<NilaiSiswaScreen> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      if (profileRes != null) {
        ...
        if (role == 'guru' && profileRes['mata_pelajaran'] != null) {
          String mapelStr = profileRes['mata_pelajaran'];
          _listMapel = mapelStr.split(',').map((e) => e.trim()).toList();
        } else {
          _listMapel = [
            'Matematika',
            'Bahasa Indonesia',
      ...
      final nilaiRes = await _supabase
          .from('nilai')
  ...
  Future<void> _simpanNilai() async {
    if (_selectedSiswa == null ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Isi data siswa, mata pelajaran, dan nilai!'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (parsedValue == null) {
        throw 'Format angka pada nilai tidak valid.';
      }
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Nilai siswa berhasil disimpan!')),
      );
      ...
      debugPrint('Error saat menyimpan nilai: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Widget build(BuildContext context) {
    // Saring list nilai berdasarkan mata pelajaran yang dipilih
    final filteredNilai = _selectedFilterMapel == null
        ? _listNilai
        : _listNilai.where((item) {
            return item['mata_pelajaran'] == _selectedFilterMapel;
          }).toList();
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Manajemen & Rekap Nilai'),
          ...
          bottom: const TabBar(
            ...
            tabs: [
              Tab(icon: Icon(Icons.add), text: 'Input Nilai'),
              Tab(icon: Icon(Icons.list_alt), text: 'Rekap Nilai'),
            ],
          ),
        ),
        body: _isLoading
                    ...
                    children: [
                      ...
                      Expanded(
                        child: filteredNilai.isEmpty
                            ? const Center(
                                child: Text(
                                  'Belum ada data nilai untuk mapel ini.',
                                ),
                              )
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Card(
                                    ...
                                    child: ListTile(
                                      ...
                                      subtitle: Text(
                                        'Mapel: ${nilai['mata_pelajaran'] ?? '-'}\nSemester: ${nilai['semester'] ?? '-'} | Kategori: ${nilai['kategori'] ?? '-'}\nNilai: ${nilai['nilai'] ?? '-'}',
                                      ),
                                      trailing: IconButton(
                                        icon: const Icon(
                                          Icons.delete,
