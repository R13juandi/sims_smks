...
class _RekapAbsensiGuruScreenState extends State<RekapAbsensiGuruScreen> {
  ...
  Future<void> _fetchGuruDanRekap() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() { _dataAbsen = List<Map<String, dynamic>>.from(res); _isLoading = false; });
      }
    } catch (e) {
                          ...
                          child: Column(
                            children: [
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Container(width: double.infinity, padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(8)), child: Text('📌 Verifikasi: $verifikasi\n📝 Keterangan: ${a['keterangan']}', style: const TextStyle(fontSize: 11, fontStyle: FontStyle.italic, color: Colors.black87, height: 1.4))),
                                      ],
                                    ),
                                  ),
                                ],
                              ...
                              Row(
                                ...
                                children: [
                                  if (verifikasi == 'Pending') ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade600, foregroundColor: Colors.white, textStyle: const TextStyle(fontSize: 12)), onPressed: () => _updateStatusAbsen(a['id'], a['status'], 'Disetujui'), icon: const Icon(Icons.check_circle, size: 16), label: Text(a['status'] == 'I' ? 'ACC IZIN' : 'ACC SAH')),
                                  if (a['status'] != 'A') ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: Colors.red.shade600, foregroundColor: Colors.white, textStyle: const TextStyle(fontSize: 12)), onPressed: () => _updateStatusAbsen(a['id'], 'A', 'Disetujui'), icon: const Icon(Icons.cancel, size: 16), label: const Text('TANDAI ALFA')),
                                ],
                              )
                            ],
                          ),
